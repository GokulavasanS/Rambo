...
// 6 distinct resume template renderers.
// Each is a pure functional component: ResumeData + ResumeTheme → JSX
// ============================================================
...
// Left-aligned, thin underline separators, navy heading
// ============================================================
...
export function TemplateClassicProfessional({ data, theme }: { data: ResumeData; theme: ResumeTheme }) {
    ...
    return (
        <div style={{ background: bg, color: text, fontFamily: theme.fontFamily, padding: '48px 52px', minHeight: '100%' }}>
            ...
            {/* Sections */}
            {data.sections.map((s) => <Section1 key={s.id} section={s} theme={theme} />)}
        </div>
    );
}

function Section1({ section, theme }: { section: ResumeSection; theme: ResumeTheme }) {
    return (
        <div style={{ marginBottom: 20 }}>
            <h2 style={{
                margin: '0 0 6px', fontSize: 11, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase',
                color: theme.palette.primary, paddingBottom: 4, borderBottom: `1.5px solid ${theme.palette.primary}`,
            }}>{section.title}</h2>
            {section.entries?.map((e) => <EntryBlock key={e.id} entry={e} theme={theme} />)}
            {section.bullets && <BulletList bullets={section.bullets} theme={theme} />}
        </div>
    );
}
...
// ============================================================
// 2. Minimal Clean
// Centered header, large whitespace, no decorations
// ============================================================
...
export function TemplateMinimalClean({ data, theme }: { data: ResumeData; theme: ResumeTheme }) {
    ...
    return (
        <div style={{ background: bg, color: text, fontFamily: theme.fontFamily, padding: '56px 64px', minHeight: '100%' }}>
            ...
            {data.sections.map((s) => <Section2 key={s.id} section={s} theme={theme} />)}
        </div>
    );
}

function Section2({ section, theme }: { section: ResumeSection; theme: ResumeTheme }) {
    return (
        <div style={{ marginBottom: 28 }}>
            <h2 style={{
                margin: '0 0 12px', fontSize: 10, fontWeight: 600, letterSpacing: 2.5, textTransform: 'uppercase', color: '#999',
                textAlign: 'center',
            }}>{section.title}</h2>
            {section.entries?.map((e) => <EntryBlock key={e.id} entry={e} theme={{ ...theme, palette: { ...theme.palette, primary: '#111', textColor: '#333' } }} />)}
            {section.bullets && <BulletList bullets={section.bullets} theme={{ ...theme, bulletStyle: 'dash' }} />}
        </div>
    );
}
...
// ============================================================
// 3. Modern ATS
// Bold orange bar headings, arrow bullets, sidebar stripe
// ============================================================
    ...
    return (
        <div style={{ background: '#fff', color: text, fontFamily: theme.fontFamily, padding: '0', minHeight: '100%' }}>
            ...
            <div style={{ padding: '28px 40px' }}>
                {data.sections.map((s) => <Section3 key={s.id} section={s} theme={theme} />)}
            </div>
        </div>
    );
}

function Section3({ section, theme }: { section: ResumeSection; theme: ResumeTheme }) {
    const accent = theme.palette.accent || '#ff6b00';
    return (
        <div style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <div style={{ width: 4, height: 18, background: accent, borderRadius: 2, flexShrink: 0 }} />
                <h2 style={{ margin: 0, fontSize: 12, fontWeight: 700, letterSpacing: 0.8, textTransform: 'uppercase', color: theme.palette.secondary || '#0f172a' }}>{section.title}</h2>
            </div>
            {section.entries?.map((e) => <EntryBlock key={e.id} entry={e} theme={theme} />)}
            {section.bullets && <BulletList bullets={section.bullets} theme={theme} />}
        </div>
...
// ============================================================
// 4. Compact Developer
// Dense, monospace feel, pill headings, indigo/purple
// ============================================================
...
export function TemplateCompactDeveloper({ data, theme }: { data: ResumeData; theme: ResumeTheme }) {
    ...
    return (
        <div style={{ background: bg, color: text, fontFamily: theme.fontFamily, padding: '32px 40px', minHeight: '100%' }}>
            ...
            {data.sections.map((s) => <Section4 key={s.id} section={s} theme={theme} />)}
        </div>
    );
}

function Section4({ section, theme }: { section: ResumeSection; theme: ResumeTheme }) {
    const primary = theme.palette.primary || '#4f46e5';
    return (
...
// ============================================================
// 5. Elegant Serif
// Centered header, serif type, warm palette, generous spacing
// ============================================================
...
export function TemplateElegantSerif({ data, theme }: { data: ResumeData; theme: ResumeTheme }) {
    ...
    return (
        <div style={{ background: bg, color: text, fontFamily: theme.fontFamily, padding: '52px 60px', minHeight: '100%' }}>
            ...
            {data.sections.map((s) => <Section5 key={s.id} section={s} theme={theme} />)}
        </div>
    );
}

function Section5({ section, theme }: { section: ResumeSection; theme: ResumeTheme }) {
    const primary = theme.palette.primary || '#6b4c2a';
    return (
...
// Left sidebar (dark) + right main content
// ============================================================
...
export function TemplateTwoColumn({ data, theme }: { data: ResumeData; theme: ResumeTheme }) {
    ...
    const sideSections: ResumeSection[] = data.sections.filter(
        (s) => s.type === 'skills' || s.type === 'education' || s.type === 'certifications'
    );
    const mainSections: ResumeSection[] = data.sections.filter(
        (s) => s.type !== 'skills' && s.type !== 'education' && s.type !== 'certifications'
    );
